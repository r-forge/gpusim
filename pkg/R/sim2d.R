...
.sim2d <- function(grid, covmodel, sill, range, nugget, k, samples, uncond, kriging.method = 'O', mu = 0, as.sp = FALSE, check = FALSE, verify = FALSE) {
	out = 0
	...
	if (!missing(uncond) && !missing(samples)) {
		...
		retcode = 0
		result = .C("unconditionalSimInit_2d", as.double(xmin), as.double(xmax), as.integer(nx), as.double(ymin),as.double(ymax), as.integer(ny), as.double(sill), as.double(range), as.double(nugget), as.integer(.covID(covmodel)), as.integer(check), retcode = as.integer(retcode), PACKAGE="gpusim")
