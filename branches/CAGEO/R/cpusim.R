...
cpuSim <- function(grid, covmodel, sill, range, nugget, k, samples, uncond, kriging.method='O', mu=0, aggregation.features=NULL, aggregation.func=mean, as.sp=FALSE, neg.eigenvals.action = "ignore",eigenvals.tol=-1e-07, benchmark=FALSE, compute.stats=FALSE, anis=c(0,0,0,1,1)) {
	...
	out <- 0
	if (dims == 2) {	
		out <- .cpusim2d(grid, covmodel, sill, range, nugget, k, samples, uncond, kriging.method, mu, aggregation.features, aggregation.func, as.sp, neg.eigenvals.action, eigenvals.tol, benchmark, compute.stats, anis)
	}
	else if (dims == 3) {
